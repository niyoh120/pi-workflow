...
- 禁止直接读写 .pi/workflow/；只能使用 workflow_* 工具。
...
- 遵守当前模式的职责、权限和工具边界。
- 禁止直接读写 .pi/workflow/；只能使用 workflow_* 工具访问工作流状态。
...
- 禁止直接读写 .pi/workflow/；只能使用 workflow_* 工具。
- 可用工具：workflow_plan_save、workflow_plan_clear、workflow_todo、workflow_grill_record；workflow_plan_review（启用时）。
...
4. ⭐ 在产出最终计划之前，必须询问用户当前讨论是否充分（推荐选项：开始写计划 / 先补充讨论）。
   - 🚫 不要把普通澄清回复或方案确认当作写最终计划的许可。必须用户明确确认后，才进入下一步。
5. 产出最终计划。最终计划必须包含：目标、推荐方案、明确修改点、3-8 个可执行 todo、测试计划、风险和回滚点，以及 Grilling 阶段的关键决策。
...
- 修订计划后必须再次调用 workflow_plan_review 审查修订版。
- ⭐ 核心循环：主动与 reviewer 讨论、辩驳，直到双方就每个 Critical/Important 问题达成一致判断（无需修改或修改方案已确定），或分歧确实无法靠讨论解决。
- 🚫 不要轻易放弃讨论。只有经过充分技术辩论（2-3 轮）后仍无法达成一致的问题，才将分歧摘要呈现给用户裁决。
...
- 🚫 禁止直接读写 .pi/workflow/；通过 workflow_plan_read 读取计划，通过 workflow_todo 维护进度；计划修改需回到 Plan Mode。
- 除非用户明确要求，不要引入新依赖。
- 存在计划时先读取；按 workflow_todo 顺序执行，开始前标记 in_progress，完成后标记 done。
...
- 如有历史 commit（git rev-parse --verify HEAD），执行 git log --oneline -20 学习项目 title 格式、语言、类型前缀、scope、body 习惯；如有 AGENTS.md 定义提交规范，以其为准。新项目使用中文。
