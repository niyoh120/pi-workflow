 ...
 */
class ScopeInputForm implements Focusable {
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, Key.tab) || matchesKey(data, Key.shift("tab"))) {
			if (matchesKey(data, Key.shift("tab"))) {
				...
				this.activeField = (this.activeField + 1) % this.inputs.length;
			}
			this.invalidate();
			return;
		}
	...
	render(theme: Theme, width: number): string[] {
		...
		for (let i = 0; i < this.inputs.length; i++) {
			...
			let line: string;
			const cursorMarker = isActive && this.focused ? CURSOR_MARKER : "";
			if (inputText.length === 0) {
				line = `${prefix} ${labelStyled} ${cursorMarker}${theme.fg("dim", "(empty)")}`;
			} else {
				line = `${prefix} ${labelStyled} ${cursorMarker}${inputText}`;
			}
		...
		lines.push(
			theme.fg(
				"dim",
				"tab/shift+tab  switch field  •  enter  confirm  •  esc  clear/cancel",
			),
		);
