...
- 两个独立的审查概念：(1) 可选 Plan Review（`workflow_plan_review`，Plan Mode 内模型按复杂度发起，`planReview.enabled` 控制）；(2) 按需统一 Review（`workflow_review`，默认开启，`review.enabled` 控制 `/review` 与工具可用性；`codeReview.enabled` 控制统一 Review 是否包含 workspace OCR）。Review 结果是瞬时工具输出，不写入 WorkflowState，也不门禁 `/commit`。
