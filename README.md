...
Plan review runs an **optional, model-initiated** independent reviewer — no subprocess, no RPC, no Quick fallback. The plan agent calls `workflow_plan_review` (optionally with `feedback`) when the plan is complex or the user requests it.
...
- **Efficient repeated calls** — actual reviewer rounds are persisted per plan run (session-scoped `plan-review-history.json`, newest 3 kept): identical repository + review basis (requirements, reviewer model/thinking, tool surface, reviewer protocol text) + task inputs (Final Plan, confirmed decisions, feedback) reuse the cached round with zero reviewer cost and append no new round; a revised plan, new confirmed decisions, or feedback run an INCREMENTAL review that injects the previous round's findings plus a Markdown section delta and re-dispositions them; changed requirements/model/tools/repository force a full review. All caching is fail-safe: unknown fingerprints, changed basis, or an unusable prior round degrade to full review.
- **Optional feedback** — the planner may pass `workflow_plan_review({ feedback })` to respond to the previous round's disputed Critical/Important findings. Feedback is injected as a clearly-labeled UNTRUSTED indented code block; the reviewer verifies every claim independently. First-round feedback is rejected with an explicit error.
- **Machine verdict** — emits `PLAN_REVIEW_VERDICT: PASS|FAIL` on the final line; fail-closed on missing/conflicting/unrecognized values. Distinct from Implementation Review's `REVIEW_VERDICT:` prefix.
- **Strict builtin repo evidence** — an effective PASS (and round cacheability) requires at least one builtin repository tool (read/bash/grep/find/ls) that COMPLETED successfully (`tool_execution_end` with `isError === false`). A parsed PASS without that evidence is downgraded to FAIL; a round with only MCP/Web file tools leaves an inspection-evidence gap the planner should treat as insufficient evidence and re-run.
...
- **vs Implementation Review** — the two reviewers keep distinct evidence/round policies: Plan Review requires strict finalized builtin repo-tool evidence and its short-circuited calls append NO new history round; Implementation Review keeps its active-tools `madeRepoToolCall` judgment and persists short-circuited rounds. `/wf-reset` clears both histories.
